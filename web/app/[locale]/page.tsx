...
const getFrontpageFields = async (locale: string) => {
  try {
    const content = await fetchAPI<FrontPageFields>(
      "/front-page",
      { cache: "no-store" },
      { locale },
    );
    ...
    <div className="container max-w-3xl bg-secondary-50 dark:bg-secondary-800 mx-auto rounded shadow-md mt-4 p-1 sm:p-8">
      <main className="container mx-auto px-4">
        <ReactMarkdown
          ...
          className="prose prose-strong:text-secondary-50 text-secondary-50 dark:prose-invert prose-li:my-0.5 prose-ul:my-0.5 prose-secondary mt-0 mb-4"
        >
          {bodyText}
        </ReactMarkdown>
        {showSignups && (
