    ...
    <div className="container max-w-3xl bg-secondary-50 dark:bg-secondary-800 mx-auto rounded shadow-md p-3 sm:p-8 text-secondary-800 dark:text-secondary-100">
      <div className="flex gap-5 mb-6">
        {itemCategories?.map((category) => (
          <div key={category.id}>
            {translation[category.attributes.name]}:{" "}
      ...
      <div className="flex border-b-4 border-b-secondary-200 dark:border-b-secondary-700 py-2 justify-around text-xl ">
        ...
        <div className="flex-1">{translation.name}</div>
        <div className="flex-1">{translation.group}</div>
      </div>
      {signups?.map((field) => (
        <div
          key={field.id}
        ...
        >
          <div className="flex-[0.5]">{field.attributes.index}</div>
          ...
          <div className="flex-1">{field.attributes.group}</div>
        </div>
      ))}
      <div className="mt-4">
