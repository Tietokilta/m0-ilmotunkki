...
const getItem = async (token: string, locale: string) => {
  ...
  try {
    const item = await serverFetchAPI<Item>(
      `/items/${itemId}`,
      {
        cache: "no-store",
      },
...
const CallbackPage = async ({ params: { token, locale } }: Props) => {
  ...
  const success = (
    <div className="bg-success-500 rounded p-8 text-center text-3xl uppercase">
      <p>{translation.success}</p>
      {item && (
        <p>
          {translation[item.attributes.itemType.data.attributes.slug]} ID:
          {item.id}
        </p>
      )}
    </div>
  ...
  const fail = (
    <div className="bg-danger-500 rounded p-8 text-center text-3xl uppercase">
      {translation.failed}
    </div>
  );
  return (
